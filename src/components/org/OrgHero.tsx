...
export default function OrgHero({ org, actions }: OrgHeroProps) {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-2xl">
      {/* Banner */}
      <div className="relative h-52 md:h-72 w-full overflow-hidden bg-gradient-to-br from-indigo-900/40 to-purple-900/40">
      ...
      <div className="absolute bottom-0 left-0 right-0 px-6 pb-6 flex flex-col sm:flex-row sm:items-end gap-4">
        ...
        <motion.div
          ...
          className="w-20 h-20 md:w-24 md:h-24 rounded-2xl border-4 border-white/20 overflow-hidden flex items-center justify-center text-3xl font-bold shadow-2xl flex-shrink-0"
        ...
        >
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {org.isHost && (
              <span className="flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-300 border border-yellow-400/30">
                <Crown size={10} /> Official S.A.N.K.A.L.P.
              </span>
            )}
          </div>
          <h1 className="text-2xl md:text-4xl font-bold text-white leading-tight">{org.name}</h1>
          {org.tagline && (
            <p className="text-white/60 text-sm md:text-base mt-0.5 line-clamp-2">{org.tagline}</p>
          )}
