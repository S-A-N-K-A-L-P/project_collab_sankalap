...
export default function ProjectProgressDashboardPage() {
    ...
    if (loading) {
        return <div className="rounded-2xl border border-border bg-card p-6 text-sm text-muted-foreground">Loading progress summary...</div>;
    }
        ...
        <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <div className="mt-3 space-y-2">
                    {members.map((member) => (
                        <div key={member.id} className="rounded-lg border border-border bg-muted-bg p-3 flex justify-between">
                            <span className="text-sm text-foreground">{member.name}</span>
                            <span className="text-xs text-muted-foreground">{member.taskCount} tasks</span>
                        </div>
                    ))}
                    {members.length === 0 && <p className="text-sm text-muted-foreground">No team members assigned yet.</p>}
                </div>
            </div>
        </div>
    );
...
function MetricCard({ label, value }: { label: string; value: number | string }) {
    return (
        <div className="rounded-lg border border-border bg-muted-bg p-4 text-center">
