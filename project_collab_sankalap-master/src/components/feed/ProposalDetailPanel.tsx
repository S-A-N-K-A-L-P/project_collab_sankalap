...
export default function ProposalDetailPanel({ proposal, onClose }: { proposal: Proposal; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Metadata grid — 2 cols, no truncation */}
      <div className="px-5 py-4 bg-background/20">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-card border border-border rounded-lg p-3 space-y-1.5">
            ...
            <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${typeColor}`}>
              {proposal.type}
            </span>
          </div>
          ...
          <div className="bg-card border border-border rounded-lg p-3 space-y-1.5">
            <div className="flex items-center gap-1.5">
              <Activity className="w-3 h-3 text-muted shrink-0" />
              <span className="text-[10px] font-semibold text-muted uppercase tracking-wider">Status</span>
            </div>
            <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${statusColor}`}>
              {proposal.status}
            </span>
          </div>
