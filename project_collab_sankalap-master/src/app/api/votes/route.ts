...
export async function POST(req: Request) {
    try {
        ...
        const now = new Date();
        if (proposal.startTime && now < new Date(proposal.startTime)) return NextResponse.json({ message: "Voting hasn't started yet" }, { status: 400 });
        if (proposal.endTime && now > new Date(proposal.endTime)) return NextResponse.json({ message: "Voting has closed" }, { status: 400 });
